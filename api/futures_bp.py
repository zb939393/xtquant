...
_MIN_VWAP_HIST_DAYS = 277   # 与 vwap_startup.min_days / baseline_min_days 一致


@futures_bp.route("/vwap/backfill", methods=["GET", "POST"])
def futures_vwap_backfill():
    """VWAP 深历史回补（运维操作，已从 app 启动钩子移出，改由运维面板 / CLI 触发）。

    GET  → 各品种 data/vwap_hist 的深历史状态（交易日数、是否齐备 ≥277、区间），只读秒回。
    POST → 后台触发一次回补（restore 接回本地备份 → 国元合肥锚点补齐缺口，union、旧值优先、
           已收盘 bar 冻结）；立即返回 started=True，不阻塞响应，进度可轮询 GET 观察 days 增长。
    深历史语义见 core/vwap_backup.py 与 core/vwap_startup.py；CLI 等价命令：
        python tools/backfill_vwap_history.py IFL9
    """
    try:
        try:
            from core import vwap_backup as vb
    ...
    if request.method == "POST":
        ...
        try:
            try:
                from core import vwap_startup as vs
            except Exception:
                import vwap_startup as vs
        except Exception as e:
            return jsonify({"ok": False, "error": "vwap_startup unavailable: %r" % (e,)})
        try:
            vs.ensure_vwap_history(codes=codes, min_days=_MIN_VWAP_HIST_DAYS, background=True)
            return jsonify({
                "ok": True, "started": True, "codes": codes,
                "msg": "已在后台启动回补（restore 接回本地备份 → 国元锚点补齐缺口），可轮询 GET 观察进度",
            })
        except Exception as e:
            return jsonify({"ok": False, "error": str(e)})
